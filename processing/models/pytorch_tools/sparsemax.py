...
class Sparsemax(nn.Module):
    ...
    def forward(self, input_):
        ...
        zs = torch.sort(input_, dim=dim, descending=True)[0]
        range_ = torch.arange(start=1, end=number_of_logits + 1, step=1, device=device, dtype=input_.dtype).view(1, -1)
    ...
    def backward(self, grad_output):
        ...
        sum_ = torch.sum(grad_output * non_zeros, dim=dim) / torch.sum(non_zeros, dim=dim)
        self.grad_input = non_zeros * (grad_output - sum_.expand_as(grad_output))
