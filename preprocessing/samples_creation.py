...
def create_samples_idiab_study(data, ph, hist, day_len):
    """
    Create samples consisting in glucose, insulin and CHO histories (past hist-length values)
    :param data: dataframe
    :param ph: prediction horizon in minutes in sampling frequency scale
    ...
    h = np.array([data.loc[i:i + n_samples - 1, "heartrate"] for i in range(hist)]).transpose()
    st = np.array([data.loc[i:i + n_samples - 1, "steps"] for i in range(hist)]).transpose()
    cpb = np.array([data.loc[i:i + n_samples - 1, "CPB"] for i in range(hist)]).transpose()
    iob = np.array([data.loc[i:i + n_samples - 1, "IOB"] for i in range(hist)]).transpose()
    aob = np.array([data.loc[i:i + n_samples - 1, "AOB"] for i in range(hist)]).transpose()
    ...
    new_data = pd.DataFrame(data=np.c_[t, g, c, i, m, cal, h, st, cpb, iob, aob, y], columns=new_columns)
    new_data["datetime"] = d
    new_data = new_data.loc[:, np.r_[["datetime"], new_columns]]  # reorder the columns, with datetime first
