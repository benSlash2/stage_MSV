...
def print_ig_sample(features, attr, results, sample=0):
    ...
    true, pred = results.iloc[sample + 1].values

    default_cmap = LinearSegmentedColormap.from_list('custom blue', [(0, '#ffffff'), (0.25, '#000000'),
                                                                     (1, '#000000')], N=256)

    _ = visualize_image_attr(attr_sample, image, method='heat_map', cmap=default_cmap, show_colorbar=True,
                             sign='absolute_value', outlier_perc=1)

    _ = visualize_image_attr_multiple(attr_sample, image, ["original_image", "heat_map", "heat_map", "heat_map"],
                                      ["all", "absolute_value", "negative", "positive"], show_colorbar=True)

    # _ = visualize_image_attr_multiple(attr_sample, image, ["original_image", "heat_map"],
    #                                   ["all", "all"], show_colorbar=True)


def print_all(features, attr, results):
    for i in range(features.shape[2]):
        print_ig_sample(features, attr, results, i)
